...
# There are 2 types of actions a agent can perform in the game
class Action(IntEnum):
    TAKE_FROM_FACTORY = 1
    TAKE_FROM_CENTRE = 2

# Bundle together a agent's activity in the game for use in
# updating a policy
class AgentTrace:
...
# collected by a agent. Note that the sum of 'num_to_pattern_line'
...
def ActionToString(agent_id, action):
    ...
    if action[0] == Action.TAKE_FROM_FACTORY:
        desc1 = "Agent {} takes {} {} tiles from factory {}".format(
            agent_id, tg.number, TileToString(tg.tile_type), action[1]+1)
    ...
    elif action[0] == Action.TAKE_FROM_CENTRE:
        desc1 = "Agent {} takes {} {} tiles from centre".format(
            agent_id, tg.number, TileToString(tg.tile_type))
...
def BoardToString(game_state):
    ...
    if game_state.first_agent_taken:
        desc += "\n"
    else:
        desc += " + first agent token (-1)\n"
